            ...
            <div>
              ...
              <label className="block mb-8 cursor-pointer">
                <div className="border-2 border-dashed border-gray-300 hover:border-blue-500 rounded-xl p-12 text-center bg-white transition-colors">
                  ...
                  <div className="text-sm text-gray-500">MP4, MOV - Max 50 MB -'use client'
            ...
            <div>
              ...
              <label className="block mb-8 cursor-pointer">
                <div className="border-2 border-dashed border-gray-300 hover:border-blue-500 rounded-xl p-12 text-center bg-white transition-colors">
                  ...
                  {uploading && (
                    <div className="mt-4">
                      <div className="w-full bg-gray-200 rounded-full h-3"><div className="bg-blue-600 h-3 rounded-full transition-all" style={{width:`${uploadProgress}%`}}></div></div>
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {['Bertrand','Sébastien','PierreEmmanuel'].map((name) => {
                          ...
                          return (<button key={name} onClick={() => toggleApproval(video.id, name)} className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${approved ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600 hover:bg-blue-50'}`}>{approved ? '✓' : '○'} {name === 'PierreEmmanuel' ? 'Pierre E.' : name}</button>)use client'
...
export default function Home() {
  ...
  async function handleVideoUpload(event) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        // 1. Obtenir l'URL signée depuis notre API
        const response = await fetch('/api/upload', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        // 2. Upload direct vers Supabase avec l'URL signée
              ...
              <label className="block mb-8 cursor-pointer">
                <div className="border-2 border-dashed border-gray-300 hover:border-blue-500 rounded-xl p-12 text-center bg-white transition-colors">
                  ...
                  <div className="text-lg font-medium text-gray-700 mb-2">
                    {uploading ? 'Upload en cours...' : 'Cliquez pour sélectionner des vidéos'}
                  </div>
                  <div className="text-sm text-gray-500">MP4, MOV • Max 50 MB • Sélection multiple possible</div>
                    ...
                    <div className="mt-4">
                      <div className="w-full bg-gray-200 rounded-full h-3">
                        <div 
                          className="bg-blue
git add .
git commit -m "Feat: signed URL upload + progress bar + multiple files"
git push origin main
cat /Users/mbpbb/skroll-tv/app/api/upload/route.js | head -20
git status
